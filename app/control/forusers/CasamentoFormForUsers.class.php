 ...
 */
class CasamentoFormForUsers extends TPage
{
    protected $form; // form
    ...
    public function __construct( $param )
    {
        ...
        $this->form->addFields([$id_usuario]);
        ...
        $horas->setTip('Selecione a hora');
        $minutos->setTip('Selecione os minutos');
    ...
    public function onEdit( $param )
    {
        try
        {
            ...
            {
                $this->form->clear(TRUE);
                $arrayclear = new stdClass;
                $arrayclear->id_usuario = TSession::getValue('userid');
                $this->form->setData($arrayclear);
            }
        }
        catch (Exception $e) // in case of exception
        {
    ...
    // Function to autoload information about user and enable a new Casamento
    public function show()
    {
        ...
        if (isset($casamento[0]->id))
        {
          $param['key'] = $casamento[0]->id;
        }
        $this->onEdit($param);
        parent::show();
    }
