 ...
 */
class LocaisListForUsers extends TPage
{
    private $form; // form
    ...
    public function __construct()
    {
        ...
        // creates the form
        $this->form = new TQuickForm('form_search_Locais');
        ...
        $this->form->setData( TSession::getValue('Locais_filter_data') );

        // add the search form actions
        $btn = $this->form->addQuickAction(_t('Find'), new TAction(array($this, 'onSearch')), 'fa:search');
        $btn->class = 'btn btn-sm btn-primary';
        $this->form->addQuickAction(_t('New'),  new TAction(array('LocaisFormForUsers', 'onEdit')), 'bs:plus-sign green');
        ...
        // create EDIT action
        $action_edit = new TDataGridAction(array('LocaisFormForUsers', 'onEdit'));
        ...
        $container->add(TPanelGroup::pack('LISTAGEM DE LOCAIS', $this->form));
    ...
    public function onSearch()
    {
        ...
        TSession::setValue('LocaisList_filter_nome',   NULL);
        TSession::setValue('LocaisList_filter_rua',   NULL);
        TSession::setValue('LocaisList_filter_bairro',   NULL);
        TSession::setValue('LocaisList_filter_municipio',   NULL);
        TSession::setValue('LocaisList_filter_estado',   NULL);
        TSession::setValue('LocaisList_filter_numero',   NULL);

        if (isset($data->nome) AND ($data->nome)) {
            $filter = new TFilter('nome', 'like', "%{$data->nome}%"); // create the filter
            TSession::setValue('LocaisList_filter_nome',   $filter); // stores the filter in the session
        }


        if (isset($data->rua) AND ($data->rua)) {
            $filter = new TFilter('rua', 'like', "%{$data->rua}%"); // create the filter
            TSession::setValue('LocaisList_filter_rua',   $filter); // stores the filter in the session
        }


        if (isset($data->bairro) AND ($data->bairro)) {
            $filter = new TFilter('bairro', 'like', "%{$data->bairro}%"); // create the filter
            TSession::setValue('LocaisList_filter_bairro',   $filter); // stores the filter in the session
        }


        if (isset($data->municipio) AND ($data->municipio)) {
            $filter = new TFilter('municipio', 'like', "%{$data->municipio}%"); // create the filter
            TSession::setValue('LocaisList_filter_municipio',   $filter); // stores the filter in the session
        }


        if (isset($data->estado) AND ($data->estado)) {
            $filter = new TFilter('estado', 'like', "%{$data->estado}%"); // create the filter
            TSession::setValue('LocaisList_filter_estado',   $filter); // stores the filter in the session
        }


        if (isset($data->numero) AND ($data->numero)) {
            $filter = new TFilter('numero', 'like', "%{$data->numero}%"); // create the filter
            TSession::setValue('LocaisList_filter_numero',   $filter); // stores the filter in the session
        }
        ...
        TSession::setValue('Locais_filter_data', $data);
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for Locais
            $repository = new TRepository('Locais');
            $limit = 10;
            // creates a criteria
            ...
            if (TSession::getValue('LocaisList_filter_nome')) {
                $criteria->add(TSession::getValue('LocaisList_filter_nome')); // add the session filter
            }


            if (TSession::getValue('LocaisList_filter_rua')) {
                $criteria->add(TSession::getValue('LocaisList_filter_rua')); // add the session filter
            }


            if (TSession::getValue('LocaisList_filter_bairro')) {
                $criteria->add(TSession::getValue('LocaisList_filter_bairro')); // add the session filter
            }


            if (TSession::getValue('LocaisList_filter_municipio')) {
                $criteria->add(TSession::getValue('LocaisList_filter_municipio')); // add the session filter
            }


            if (TSession::getValue('LocaisList_filter_estado')) {
                $criteria->add(TSession::getValue('LocaisList_filter_estado')); // add the session filter
            }


            if (TSession::getValue('LocaisList_filter_numero')) {
                $criteria->add(TSession::getValue('LocaisList_filter_numero')); // add the session filter
            }
    ...
    public function Delete($param)
    {
        try
        {
            ...
            TTransaction::open('organizamc'); // open a transaction with database
            $object = new Locais($key, FALSE); // instantiates the Active Record
